...
select sum(case when cc.`Primary Type` = 'WEAPONS VIOLATION' then 1 else 0 end)*100/count(cc.ID) as percent_shoot, count(cc.ID)*100/c.total_population as Crime_rate, cc.`Community Area`, c.`Area`
from census as c
left join chicrime as cc
on c.community_area_number = cc.`Community Area`
where cc.`Year` = 2015 
